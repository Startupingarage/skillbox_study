...
for n in range(N):
    ...
    elem = (-1)**n * 1/(2**n)
    summ_elem += elem
    #print(f'elem = (−1) ** {n} * (½) ** {n} = {elem}')
    #как будто в примере n = 1 # elem = (−1) ** 1 * (½) ** 1 = (−½) это указано просто для понимания, 
    # не нужно жа это в цикле прописывать принтом? Но если это не прописывать, выглядит как задача из под видео, супер легко.
    #Но с другой стороны тут прописаны значения elem в виде дроби (−⅛)
