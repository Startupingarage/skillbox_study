...
# и составляют expenses рублей в месяц. 
...
costs = int(input('Введите расходы на проживание: '))

total_shortage = 0

for i in range(1,11):
    month_cost = costs - educational_grant

    if month_cost > 0:
        total_shortage += month_cost
        print(f'{i}. месяц траты {costs:.1f} не хватает {total_shortage:.2f}')

    else:
        print(f'{i}. месяц траты {costs:.1f} средств хватает')

    costs *= 1.03

print(f"Нужно попросить у родителей {total_shortage:.2f} рублей" if total_shortage > 0 else "Денег хватает, у родителей просить не надо.")
